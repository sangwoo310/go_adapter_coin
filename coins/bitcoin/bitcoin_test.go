...
func CreateTx(privKey string, destination string, amount int64) (string, error) {
	...
	//// wif 값은 private key 의 byte 및 정보데이터
	...
	fmt.Println("destinationAddr String : ", destinationAddr.String())
	//fmt.Println("destinationAddr Encode : ", destinationAddr.EncodeAddress())
	//fmt.Println("destinationAddr Script : ", destinationAddr.ScriptAddress())
	//fmt.Println("destinationAddr ForNet : ", destinationAddr.IsForNet(&chaincfg.TestNet3Params))
	...
	fmt.Println("utxoHash txid : ", txid)
	...
	fmt.Println("redeemTx2 : ", redeemTx)
...
func SignTx(privKey string, pkScript string, redeemTx *wire.MsgTx) (string, error) {
	...
	fmt.Println("redeem : ", *redeemTx.TxIn[0])
